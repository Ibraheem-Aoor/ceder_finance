...
class BankAccountController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create bank account')) {
            ...
            $account->use_on_invoice = $request->use_on_invoice == 'on';
            $account->created_by = \Auth::user()->creatorId();
            $account->save();
            // Only One Account Should Be Used On Invoice
            if ($account->use_on_invoice == 'on') {
                BankAccount::query()->where('id', '!=', $account->id)->update(['use_on_invoice' => 0]);
            }
    ...
    public function update(Request $request, BankAccount $bankAccount)
    {
        if (\Auth::user()->can('create bank account')) {
            ...
            $bankAccount->use_on_invoice = $request->use_on_invoice == 'on';
            $bankAccount->created_by = \Auth::user()->creatorId();
            $bankAccount->save();
            // Only One Account Should Be Used On Invoice
            if ($bankAccount->use_on_invoice == 'on') {
                BankAccount::query()->where('id', '!=', $bankAccount->id)->update(['use_on_invoice' => 0]);
            }
