...
class InvoiceController extends Controller
{
    ...
    public function previewInvoice($template, $color)
    {
        ...
        $bank_account = BankAccount::where('created_by', $objUser->creatorId())->isForInvoice(true)->first();
    ...
    public function invoice($invoice_id)
    {
        ...
        foreach ($invoice->items as $product) {
            ...
            $item->unit = ProductService::query()->with('unit:id,name')->find($product->product_id)->unit?->name;
        ...
        if ($invoice) {
            ...
            $font_color = Utility::getFontColor($color);
            $bank_account = BankAccount::where('created_by', \Auth::user()->creatorId())->isForInvoice(true)->first();

            return view('invoice.templates.' . $settings['invoice_template'], compact('invoice', 'color', 'settings', 'customer', 'img', 'font_color', 'customFields' , 'bank_account'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
