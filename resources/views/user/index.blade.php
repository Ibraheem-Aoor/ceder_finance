    ...
    @can('create user')
        <div class="all-button-box row d-flex justify-content-end">
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                <a href="#" data-url="{{ route('users.create') }}" data-ajax-popup="true"
                    data-title="{{ __('Create New User') }}" class="btn btn-xs btn-white btn-icon-only width-auto">
        ...
        @foreach ($users as $user)
            <div class="col-lg-3 col-sm-6 col-md-6">
                <div class="card profile-card">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        @can('edit user')
                                            <a href="#" class="dropdown-item text-sm"
                                                ...
                                                data-title="{{ __('Update User') }}">{{ __('Edit') }}</a>
                                        @endcan
                                        @can('delete user')
                    ...
                    <h6 class="office-time mb-0 my-2">{{ $user->email }}</h6>
                    <h6>
                        @if ($user->is_accepted)
                    ...
                    <small class="my-2" data-toggle="tooltip" data-placement="bottom"
                        data-original-title="{{ __('Last Login') }}">{{ !empty($user->last_login_at) ? $user->last_login_at : '-' }}</small>
                    @if (\Auth::user()->type == 'super admin')
                        <div class="mt-4">
                            <div class="row justify-content-between align-items-center">
                                <div class="col-6 text-center">
                                    <span
                                        class="d-block font-weight-bold mb-0">{{ !empty($user->currentPlan) ? $user->currentPlan->name : '' }}</span>
                                </div>
                                ...
                                <div class="col-4 text-center">
                                    <span
                                        class="d-block text-sm font-weight-bold mb-0">{{ $user->totalCompanyUser($user->id) }}</span>
